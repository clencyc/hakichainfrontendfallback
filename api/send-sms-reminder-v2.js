// Enhanced SMS API for reminder system - Express.js version
...
const SMS_SENDER_ID = process.env.TIARA_SMS_SENDER_ID || 'CONNECT';
const SMS_FALLBACK_SENDER = process.env.TIARA_SMS_FALLBACK_SENDER || 'CONNECT';

// Use fetch for API calls (node-fetch v3 ESM compatibility)
...
async function sendSingleSMS(phoneNumber, message) {
  ...
  try {
    ...
    const response = await fetch(TIARA_SMS_URL, {
      method: 'POST',
      headers: {
...
async function sendSmsReminderV2Handler(req, res) {
  // Enable CORS
...
module.exports = sendSmsReminderV2Handler;
